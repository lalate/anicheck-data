...
# anime_id 生成
# =================================================================
def generate_anime_id(title: str, yyyymm: str) -> str:
    """タイトルから一意な anime_id を生成する。

    形式: {YYYYMM}_gen_{sha256[:8]}
    例  : 202604_gen_a1b2c3d4
    """
    digest = hashlib.sha256(title.encode("utf-8")).hexdigest()[:8]
    return f"{yyyymm}_gen_{digest}"
...
def main() -> None:
    ...
    for idx, anime in enumerate(grok_anime_list):
        ...
        # Step 5b: anime_id 生成
        anime_id = generate_anime_id(title, season_info["yyyymm"])
