...
# 厳格な制約（ハルシネーション防止）
- 必ず提供された情報（公式サイト、スケジュール、検索結果）のみに基づき抽出してください。推測や外部知識の付加は厳禁です。
- 不明な情報がある場合は、無理に捏造せず、null または空文字を設定し、ソース確認の備考欄にその旨を記載してください。

# 処理ステップ（Chain of Thought）
ステップ1: 入力された作品名、話数、参考URL、基本スケジュールを分析してください。
ステップ2: Web検索（live_search）を用いて、対象話数の正確な「サブタイトル」「あらすじ」「特番による時間変更/休止の有無」「関連グッズ」「原作の該当巻数」「公式ハッシュタグ」「YouTube予告ID」を調査・抽出してください。
ステップ3: 抽出した情報を元に、指定された3つのJSONブロックを構築してください。

# 出力形式
以下の3つのJSONブロックと、その後に【ソース確認】セクションを出力してください。
